...
pdf(paste('pkbg', '.atac.merged.box.pdf', sep=''), width=6, height=4)
p = ggplot(data = frip_2c, aes(x=Method_regions, y=log2(Mean_sig))) 
p = p + geom_boxplot(aes(fill = Method_regions))
p = p + geom_point(aes(y=log2(Mean_sig), group=Method_regions), position = position_dodge(width=0.3))
p = p + scale_fill_manual(values=rep(rev(brewer.pal(8,"Set3")[1:5]),each = 2))
