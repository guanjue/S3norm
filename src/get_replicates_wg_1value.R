...
filename_rep1 = paste(ct, 'rep1.atac.meanrc.', m, '.sort.txt', sep='')
filename_rep2 = paste(ct, 'rep2.atac.meanrc.', m, '.sort.txt', sep='')
drep1 = read.table(filename_rep1, header=F)[,4]
drep2 = read.table(filename_rep2, header=F)[,4]
set.seed(2018)
used_id = sample(length(drep1), 20000)
d = cbind(drep1, drep2)[used_id,]+1
plot_heatscatter(log2(d), paste('replicate_cor/', filename, '.wg.png', sep=''))
dm = rowMeans(d)
R_vec = cor(log2(d[,1]), log2(d[,2]))
R_mat = c(R_mat, R_vec)
dat = cbind(log2(d[,1]), log2(d[,2]))
#R_vec = est.IDR(dat, mu=3, sigma=1, rho=.9, p=.5)
#pk_idr = R_vec$IDR
#R_mat = c(R_mat, mean(pk_idr))
}
output_name = paste('R_mat.', m, '.R.pr.wg.txt', sep='')
...
pdf(paste('replicate_cor/replicates_R.wg.pdf', sep=''), width=5.5, height=4)
