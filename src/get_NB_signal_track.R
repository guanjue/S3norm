...
#time Rscript get_NB_signal_track.R MK_imm_ad.atac.meanrc.S3norm.sort.txt MK_imm_ad.atac.meanrc.S3norm.sort.bg.bedgraph atac_commonpk.txt.cbg.sort.txt MK_imm_ad.atac.meanrc.S3norm.sort.NBP.bedgraph MK_imm_ad.atac.meanrc.S3norm.sort.NBQ.bedgraph
...
nb_pval = apply(sig_input, MARGIN=1, function(x) get_pval(x[1], bin_num, sigcbg_size * (x[2])/(input_mean), sigcbg_prob, obs_0_num) )
...
### write output
write.table(cbind(bed, neglog10_nb_pval), output_name1, quote=FALSE, col.names=FALSE, row.names=FALSE, sep='\t')
write.table(cbind(bed, neglog10_nb_pval_fdr), output_name2, quote=FALSE, col.names=FALSE, row.names=FALSE, sep='\t')
write.table(info_matrix, paste(output_name1, '.mvsp.txt', sep=''), quote=FALSE, col.names=FALSE, row.names=FALSE, sep='\t')
