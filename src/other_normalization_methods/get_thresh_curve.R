...
for (i in thresh_vec){
	...
	pk_len = sum(d[,3]-d[,2])
	pk_num = dim(d)[1]
	pk_info_region = rbind(pk_info_region, c(i, pk_len))
	pk_info_pk = rbind(pk_info_pk, c(i, pk_num))
}
...
for (i in thresh_vec){
	...
	pk_len = sum(d[,3]-d[,2])
	pk_num = dim(d)[1]
	pk_info_region = c(pk_info_region, pk_len)
	pk_info_pk = c(pk_info_pk, pk_num)
}
pk_info_all = cbind(pk_info_all, pk_info_region)
pk_info_pkn_all = cbind(pk_info_pkn_all, pk_info_pk)
}
...
get_NB_thresh = function(x, fdr_thresh){
top = 0.95
