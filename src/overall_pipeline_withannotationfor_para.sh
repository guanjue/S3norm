...
do
	...
	if [ "$sig1" = "$sig2" ]
	then
		cat $sig1 | awk -F '\t' -v OFS='\t' -v ul=$upperlim '{if ($1>=ul) print ul; else print $1}' > $sig1'.'$sig2'.upperlim.txt'
	else
		cat $sig1 | awk -F '\t' -v OFS='\t' -v ul=$upperlim '{if ($1>=ul) print ul; else print $1}' > $sig1'.'$sig2'.upperlim.txt'
		cat $sig2 | awk -F '\t' -v OFS='\t' -v ul=$upperlim '{if ($1>=ul) print ul; else print $1}' > $sig2'.upperlim.txt' 
	fi
	### peak norm
	if time python $script_dir's3norm.py' -r $sig1'.'$sig2'.upperlim.txt' -t $sig2'.upperlim.txt' -m 1 -i 2 -f 0.05 -n $plot_num -l $rank_lim -a 100 -b 0 -s $script_dir -p p -c T; then echo 's3norm across datasets DONE'; else echo 'ERROR: s3norm across datasets' && exit 1; fi
	### rm tmp files
	if [ "$sig1" = "$sig2" ]
	then
		rm $sig1'.'$sig2'.upperlim.txt'
	else
		rm $sig1'.'$sig2'.upperlim.txt'
...
do
	...
	do
		...
		### peak norm
		if time python $script_dir's3norm.py' -r $sig1'.'$sig2'.upperlim.txt' -t $sig2'.upperlim.txt' -m 1 -i 2 -f 0.05 -n $plot_num -l $rank_lim -a 100 -b 0 -s $script_dir -p p -c F; then echo 's3norm across datasets DONE'; else echo 'ERROR: s3norm across datasets' && exit 1; fi
		### rm tmp files
		if [ "$sig1" = "$sig2" ]
		then
			rm $sig1'.'$sig2'.upperlim.txt'
