...
method_list = method_list[c(1,4,5)]
...
for (c in ct_list){
### get input data
filename = paste(c, '_atac_macs_pk/', c, '.atac.meanrc.', m, '.macs2pk.NBQ.', t, '.txt', sep='')
print(filename)
...
method_colors = method_colors[c(1,4,5)]
### plotting function
...
### plotting
get_mergeplot(pkn_list_mat, ct_list_plotting, paste('barplot_pkn_pkr/atac.meanrc.allmethods.macs2pk.NBQ.', t, '.pkn.pdf', sep=''), method_colors, pkn_ylim)
get_mergeplot(pkr_list_mat, ct_list_plotting, paste('barplot_pkn_pkr/atac.meanrc.allmethods.macs2pk.NBQ.', t, '.pkr.pdf', sep=''), method_colors, pkr_ylim)
