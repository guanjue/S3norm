...
for (i in thresh_vec){
	...
	pk_info_pkn = rbind(pk_info_pkn, c(i, pk_num))
}
pk_info_all = cbind(pk_info_all, pk_info)
pk_info_all_pkn = cbind(pk_info_all_pkn, pk_info_pkn)
### other methods
...
for (i in thresh_vec){
	...
	pk_info_pkn = c(pk_info_pkn, pk_num)
}
pk_info_all = cbind(pk_info_all, pk_info)
pk_info_all_pkn = cbind(pk_info_all_pkn, pk_info_pkn)
}
...
write.table(pk_info_all, paste(ct, '.pk_region.txt', sep=''), quote=F, col.names=F, row.names=F, sep='\t')
write.table(pk_info_all_pkn, paste(ct, '.pk_num.txt', sep=''), quote=F, col.names=F, row.names=F, sep='\t')
...
library(mixtools)
get_gmm_thresh = function(x){
derivative_vector = as.vector(x)
gmm_derivative_1d = normalmixEM(derivative_1d, k=2)
bgc = which.min(gmm_derivative_1d$mu)
bg_binary = (gmm_derivative_1d$posterior[,bgc] >=0.5)
gmm_thresh = max(derivative_1d[bg_binary])
return(gmm_thresh)
}
...
gamma_thresh = get_gmm_thresh(derivative_1d)
