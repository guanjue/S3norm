...
for (i in 1:length(ct_list)){
	...
	ct_rep1 = paste(ct_list[i], 'rep1.', mk_list[i], '.frip.txt', sep='')
	ct_rep2 = paste(ct_list[i], 'rep2.', mk_list[i], '.frip.txt', sep='')
	ct_rep12merge_a = as.matrix(read.table(ct_rep12merge, header=F))
	ct_rep1_a = as.matrix(read.table(ct_rep1, header=F))
	ct_rep2_a = as.matrix(read.table(ct_rep2, header=F))
	merge_frips = cbind(merge_frips, ct_rep12merge_a)
	rep1_frips = cbind(rep1_frips, ct_rep1_a)
	rep2_frips = cbind(rep2_frips, ct_rep2_a)
}


rownames(merge_frips) = methods
rownames(rep1_frips) = methods
rownames(rep2_frips) = methods
colnames(merge_frips) = ct_list
colnames(rep1_frips) = ct_list
colnames(rep2_frips) = ct_list
merge_frips = t(merge_frips)
rep1_frips = t(rep1_frips)
rep2_frips = t(rep2_frips)
...
rep1_frips_2c = get_frip_2c(rep1_frips, '_rep1')
rep2_frips_2c = get_frip_2c(rep2_frips, '_rep2')

frip_2c = data.frame()
for (i in 1:length(methods)){
	used_id = (1+(i-1)*length(ct_list)):(length(ct_list)+(i-1)*length(ct_list))
	frip_2c = rbind(frip_2c, merge_frips_2c[used_id,])
	frip_2c = rbind(frip_2c, rep1_frips_2c[used_id,])
	frip_2c = rbind(frip_2c, rep2_frips_2c[used_id,])
}


frip_2c$Method = factor(frip_2c$Method, levels = unique(frip_2c$Method),ordered = TRUE)

pdf(paste('frip', '.box.pdf', sep=''), width=8, height=4)
...
p = p + scale_fill_manual(values=rep(rev(brewer.pal(8,"YlGnBu")[1:5]),each =3))
