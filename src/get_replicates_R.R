...
R_vec = c()
interval = 0.01
thresh_vec = rev(seq(0.8,1-interval, interval))
for (i in thresh_vec){
used_id = (dm > quantile(dm, i)) & (dm <= quantile(dm, i+interval))
R_tmp = cor(log2(d[,1]+1), log2(d[,2]+1))
R_vec = c(R_vec, R_tmp)
}
...
###
rownames(R_mat) = ct_list
colnames(R_mat) = thresh_vec
R_mat_2c_m = get_frip_2c(R_mat, m)
R_mat_2c = rbind(R_mat_2c, R_mat_2c_m)
}


pdf(paste('replicate_cor/replicates_R.pdf', sep=''), width=20, height=4)
...
p = p + scale_fill_manual(values=rep(c('gray', 'seagreen1', 'plum1', 'orange1', 'dodgerblue1'), each=length(thresh_vec)))
...
plot(d[used_id,1], d[used_id,2], log='xy', xlim=c(1,1000), ylim=c(1,1000))
abline(0,1)
dev.off()
